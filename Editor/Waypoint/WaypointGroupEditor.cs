...
namespace Devdog.QuestSystemPro.Editors
{
    ...
    public class WaypointGroupEditor : Editor
    {
        ...
        private static bool _holdingDeleteKey => Event.current.control;
        ...
        private static void HandlePositionHandle(WaypointGroup group)
        {
            ...
            {
                if (group != _selectedWaypointGroup || i != _selectedWaypointIndex) continue;
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    waypoint.transform.localPosition = group.transform.worldToLocalMatrix *
                                                       (waypointWorldPosition - group.transform.localPosition);
        ...
        protected static void DrawWaypoints(WaypointGroup group, GizmoType type)
        {
            ...
            for (var i = 0; i < group.waypoints.Length; i++)
            {
                ...
                {
                    if (i + 1 < group.waypoints.Length) nextWaypoint = group.waypoints[i + 1];
                }
                ...
                if (group == _selectedWaypointGroup && i == _selectedWaypointIndex) Handles.color = Color.green;
                ...
                if (nextWaypoint != null && group.waypoints.Length > 1)
                {
                    ...
                    var left = Quaternion.LookRotation(waypointDirection) * Quaternion.Euler(0f, 30f, 0f) *
                               new Vector3(0f, 0f, 0.5f);
                    var right = Quaternion.LookRotation(waypointDirection) * Quaternion.Euler(0f, -30f, 0f) *
                                new Vector3(0f, 0f, 0.5f);
        ...
        private static void HandleInput(WaypointGroup group)
        {
            ...
            for (var i = 0; i < group.waypoints.Length; i++)
            {
                ...
                if (Event.current.shift)
                {
                    ...
                    var dot = Vector3.Dot(waypointDirectionScreenSpaceDir,
                        (mouseScreenPos - waypointScreenPos).normalized);

                    var isCursorInBetweenWaypoints =
                        mouseScreenPos.x > Mathf.Min(waypointScreenPos.x, nextWaypointScreenPos.x) &&
                    ...
                    {
                        AddWaypointToGroupOnClick(group, group.waypoints.Length,
                            GetWorldpsacePositionRaycastMouseScreenPos(camera));
                        return;
                    }
                }
        ...
        private static void AddWaypointToGroupOnClick(WaypointGroup group, int afterIndex, Vector3 worldPosition)
        {
            ...
            if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
            {
                ...
                group.InsertWaypointAfter(
                    group.transform.worldToLocalMatrix * (worldPosition - group.transform.localPosition), afterIndex);
