...
namespace Devdog.General.Editors.GameRules
{
    public class DialogueNodesRule : GameRuleBase
    {
        ...
        {
            var dialogues = Resources.FindObjectsOfTypeAll<Dialogue>();
            ...
            foreach (var node in dialogue.nodes)
            {
                ...
                if (v.validationType != ValidationType.Valid)
                {
                    var d = dialogue;
                    var n = node;
                    issues.Add(new GameRuleIssue(v.message,
                        v.validationType == ValidationType.Warning ? MessageType.Warning : MessageType.Error,
                        new GameRuleAction("Select node",
                            () =>
                            {
