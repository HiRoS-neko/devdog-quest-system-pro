...
namespace Devdog.QuestSystemPro.Editors
{
    public class QuestGiverRule : GameRuleBase
    {
        ...
        {
            var questGivers = Object.FindObjectsOfType<Component>().Where(o => o is IQuestGiver);
            foreach (var questGiver in questGivers)
            {
                ...
                if (iQuestGiver.quests.Any(o => o == null))
                {
                    ...
                    issues.Add(new GameRuleIssue("Quest giver contains an empty quest", MessageType.Error,
                        new GameRuleAction("Fix (remove)",
                            () => { iQuestGiverTemp.quests = RemoveNullFromArray(iQuestGiverTemp.quests, qTemp); }
                        ), new GameRuleAction("Select", () => { SelectObject(qTemp); })));
                }
            }
        }
    }
