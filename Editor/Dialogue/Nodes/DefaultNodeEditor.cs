    ...
    public class DefaultNodeEditor : NodeEditorBase
    {
        ...
        public override void Draw(bool drawContents)
        {
            try
            {
                ...
                {
                    var elementRect = new Rect(NodeInnerPadding, NodeInnerPadding, nodeSize.x - NodeInnerPadding * 2f,
                        ReflectionDrawerStyles.singleLineHeight);
                    GUI.Label(elementRect, "#" + node.index + " " + ObjectNames.NicifyVariableName(node.GetType().Name),
        ...
        protected virtual void DrawFields(ref Rect elementRect)
        {
            ...
            {
                if (ShouldShowDrawerInNode(drawer)) continue;
            ...
            if (node.edges.Length > maxOutgoingEdges)
            {
                ...
                while (l.Count > maxOutgoingEdges) l.RemoveAt(l.Count - 1);
        ...
        protected virtual void DrawSingleField(ref Rect elementRect, DrawerBase drawer)
        {
            ...
            if (EditorGUI.EndChangeCheck()) NotifyFieldChanged(drawer);
        }
        ...
        public override void DrawSidebar(ref Rect rect)
        {
            foreach (var drawer in drawers)
            {
                if (drawer.hideInProperties) continue;
