...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    public abstract class NodeEditorBase
    {
        ...
        private readonly List<FieldInfo> _childFields = new();

        private GUIStyle _edgeConnectorHelperStyle;
        private DebugNodeVisualType _lastDebugDebugType;
        private double _lastDebugViewTime;


        private readonly Dictionary<DrawerBase, bool> _showInNodeDict = new();
        protected List<DrawerBase> drawers = new(0);

        protected int maxOutgoingEdges = 999;
        public Vector2 nodeSize = new(200f, 100f);
        public NodeBase node { get; protected set; }
        public DialogueEditorWindow editor { get; protected set; }

        public GUIStyle edgeConnectorStyle => "HelpBox";
        ...
        public GUIStyle edgeConnectorHelperStyle
        {
            get
            {
                ...
                {
                    _edgeConnectorHelperStyle = new GUIStyle("HelpBox");
        ...
        public virtual void Init(NodeBase node, DialogueEditorWindow editor)
        {
            ...
            foreach (var drawer in drawers)
                _showInNodeDict[drawer] =
                    drawer.fieldInfo.GetCustomAttributes(typeof(ShowInNodeAttribute), true).Length == 0;
        }

        protected bool ShouldShowDrawerInNode(DrawerBase drawer)
        {
            if (_showInNodeDict.ContainsKey(drawer) == false) return false;
        ...
        {
            var l = new EdgeConnector[Mathf.Min(node.edges.Length * 2 + 1,
                maxOutgoingEdges == node.edges.Length ? maxOutgoingEdges : node.edges.Length * 2 + 1)];

            var r = new Rect(node.editorPosition.x + nodeSize.x / 2f - EdgeConnectorSize / 2f * l.Length,
                node.editorPosition.y + nodeSize.y, EdgeConnectorSize, EdgeConnectorSize);
            r.position += editor.nodesOffset;
            var index = 0;
            for (var i = 0; i < l.Length; i++)
            {
                var isHelper = i % 2 == 0;
                if ((maxOutgoingEdges == node.edges.Length || isHelper) && i > 0) index++;
        ...
        public virtual EdgeConnector[] GetReceivingEdgeConnectors()
        {
            ...
            {
                new(0, 0,
                    new Rect(node.editorPosition.x + nodeSize.x / 2f - EdgeConnectorSize / 2f + editor.nodesOffset.x,
                        node.editorPosition.y - EdgeConnectorSize + editor.nodesOffset.y, EdgeConnectorSize,
                        EdgeConnectorSize), false, this)
            };
        ...
        protected virtual void DrawValidation(ValidationType validationType, string message)
        {
            ...
            else if (validationType == ValidationType.Error) EditorGUI.LabelField(r, GUIContent.none, "CN EntryError");

            if (r.Contains(Event.current.mousePosition)) DrawTooltip(Event.current.mousePosition, message);
        }
        ...
        public virtual void DrawEdges()
        {
            ...
            if (maxOutgoingEdges == node.edges.Length)
                ...
                for (var i = 1; i < node.edges.Length * 2 + 1; i += 2)
                {
                    if (i >= connectors.Length) break;
        ...
        protected virtual void DrawEdge(uint i, Rect fromRect)
        {
            ...
            {
                Debug.LogWarning("Out of range : total of " + node.edges.Length + " edges - index: " + i +
                                 " node type + " + node.GetType() + " - Node index: " + node.index);
        ...
        protected virtual Rect GetSummaryRect(IEnumerable<EdgeConditionSummary> msg, Vector2 from, Vector2 to)
        {
            ...
            foreach (var condition in msg)
            {
                ...
                if (textWidthTemp > textWidth) textWidth = textWidthTemp;
            }
        ...
        protected void DoDrawEdgeSummary(IEnumerable<EdgeConditionSummary> msg, Rect rect)
        {
            ...
            {
                if (canUseAll == false) GUI.color = Color.red;
            }
...
//            Debug.Log("Connect " + node.index + " to " + connectToNode.index);
            if (connector.isHelperConnector == false && connector.index <= node.edges.Length - 1)
            {
                ...
                if (existingNode != null) existingNode.toNodeIndex = connectToNode.index;
            }
            else
            {
