...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    ...
    public class DialogueOwnerEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            {
                var path = EditorUtility.SaveFilePanelInProject("Save location", "Dialogue" + DateTime.Now.ToFileTime(),
                    "asset", "Save new dialogue object location");
                if (string.IsNullOrEmpty(path) == false)
                {
                    var asset = CreateInstance<Dialogue>();
//                    string assetPathAndName = AssetDatabase.GenerateUniqueAssetPath(path + "/New " + typeof(Dialogue).ToString() + ".asset");
            ...
            if (GUILayout.Button("Edit dialogue")) DialogueEditorWindow.Edit(t);
            ...
            if (trigger == null)
            {
                ...
                {
                    var tempType = type;
                    if (GUILayout.Button("Add: " + tempType.Name)) t.gameObject.AddComponent(tempType);
                }
            }
        }
    }
