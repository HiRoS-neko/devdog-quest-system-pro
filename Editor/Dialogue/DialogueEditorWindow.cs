...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    public class DialogueEditorWindow : BetterEditorWindow
    {
        ...
        private const int AddNodesListWidth = 300;
        private const int AddNodesListHeight = 400;
        ...
        public static List<DialogueEditorSidebarBase> sidebars = new();

        protected static Dictionary<string, List<Type>> addNodesListLookup = new();
        protected static List<string> addNodesListExpandedKeys = new();
        private static Rect _addNodesListRect;
        private static DialogueEditorSidebarBase _selectedSidebar;

        private static bool _isListeningForDialogueChange;

        private static bool _isDirty;

        private static List<Type> _addNodesListSearchedTypes = new();
        private static int _addNodesListSelectedIndex;
        ...
        private bool _isDraggingSidebarWidth;
        ...
        private List<NodeEditorBase> _selectedNodeEditors = new();
        private bool _showAddNodesList;
        private Vector2 _showAddNodesListPosition;
        public List<NodeBase> clipboard = new();

        public List<NodeEditorBase> nodeEditors = new();
        ...
        public Dialogue dialogue
        {
            ...
            set
            {
                if (ReferenceEquals(_dialogue, value)) return; // Same, no need to change

                if (_dialogue != null) _dialogue.OnCurrentNodeChanged -= Dialogue_OnCurrentNodeChanged;
        ...
        {
            get => _selectedNodeEditors;
            protected set
            {
                foreach (var node in _selectedNodeEditors) node.UnSelect();

                value.RemoveAll(o => o == null);
                _selectedNodeEditors = value;

                foreach (var node in _selectedNodeEditors) node.Select();
            }
        }

        public static DialogueEditorWindow window { get; protected set; }

        public Rect sidebarResizableAreaRect => new(position.width - sidebarWidth - 20f, 0f, 20f, position.height);
        ...
        public override void OnGUI()
        {
            ...
            if (LocalizationManager.instance == null || LocalizationManager.defaultDatabase == null ||
                LocalizationManager.GetAvailableLanguageDatabases().Length == 0)
            {
                GUI.Label(
                    new Rect(position.width / 2f - 300f, position.height / 2f - 10f, 600f,
                        EditorGUIUtility.singleLineHeight),
                    "No localization manager / databases found. The localization database saves all localizable data.",
                    EditorStyles.wordWrappedLabel);

                if (GUI.Button(
                        new Rect(position.width / 2f - 100f, position.height / 2f + 10f, 200f,
                            EditorGUIUtility.singleLineHeight), "Open setup wizard")) GameRulesWindow.ShowWindow();
                return;
            }
            ...
            if (EditorApplication.isPlaying)
            {
                ...
                {
                    if (GUI.Button(new Rect(300, 0, 150, 24),
                            new GUIContent("[Start] auto change",
                                "Automatically change the dialogue when the player uses one in-game.")))
                    {
            ...
            else if (_isDrawingSelectionGrid && _isDraggingSidebarWidth == false) DrawSelectionRect();
            ...
            if (_showAddNodesList) DrawAddNodesList();
            ...
            using (new ScrollableBlock(
                       ...
                       200))
            {
                using (new GroupBlock(new Rect(0f, 0f, sidebarWidth, position.height), GUIContent.none,
...
//            Debug.Log("Scripts reloaded");
        ...
        private static void CreateSidebars()
        {
            ...
            if (_selectedSidebar == null) _selectedSidebar = sidebars[0];
        }
        ...
        public static void PingNode(NodeBase node)
        {
            ...
            if (selectedEditor != null) selectedEditor.PingDebugView(DebugNodeVisualType.Debug);
        }
        ...
        {
            if (window.dialogue == null) return;
            ...
            foreach (var node in window.dialogue.nodes) window.nodeEditors.Add(window.CreateEditorForNode(node));
        ...
        public static void UpdateAvailableNodeTypes()
        {
            ...
            {
                var category =
                    (CategoryAttribute)n.GetCustomAttributes(typeof(CategoryAttribute), true).FirstOrDefault();
                var hideInSidebar =
                    (HideInCreationWindowAttribute)n.GetCustomAttributes(typeof(HideInCreationWindowAttribute), true)
                        .FirstOrDefault();
                if (hideInSidebar != null) continue;

                var categoryName = "Undefined";
                if (category != null) categoryName = category.category;
        ...
        private void DrawAddNodesList()
        {
            ...
            _addNodesListRect = new Rect(_showAddNodesListPosition + Vector2.one * 5f,
                new Vector2(AddNodesListWidth, AddNodesListHeight));
            ...
            if (isSearching && Event.current.type == EventType.KeyDown)
            {
                ...
                else if (Event.current.keyCode == KeyCode.UpArrow) _addNodesListSelectedIndex--;
            }
            ...
            using (new GroupBlock(_addNodesListRect, GUIContent.none, "box"))
            {
                ...
                _addNodesListSearch =
                    General.Editors.EditorStyles.SearchBar(rect, _addNodesListSearch, this, isSearching);
                GUI.FocusControl("SearchField");

                rect = new Rect(new Vector2(5f, EditorGUIUtility.singleLineHeight + buttonMargin * 2f),
                    new Vector2(AddNodesListWidth - 10f, AddNodesListHeight - EditorGUIUtility.singleLineHeight));

                foreach (var kvp in addNodesListLookup)
                foreach (var type in addNodesListLookup[kvp.Key])
                    if (isSearching)
                    {
                        if (type.Name.ToLower().Contains(_addNodesListSearch.ToLower()) == false) continue;

                        _addNodesListSearchedTypes.Add(type);
                    }

                _addNodesListSelectedIndex = Mathf.Clamp(_addNodesListSelectedIndex, 0,
                    Mathf.Max(0, _addNodesListSearchedTypes.Count - 1));
                _addNodesListSearchedTypes = _addNodesListSearchedTypes.OrderBy(o =>
                    LevenshteinDistance(o.Name.ToLower(), _addNodesListSearch.ToLower()) + o.Name.Length -
                    _addNodesListSearch.Length).ToList();
                using (new GroupBlock(rect, GUIContent.none))
                {
                    if (isSearching)
                        for (var i = 0; i < _addNodesListSearchedTypes.Count; i++)
                        {
                            var type = _addNodesListSearchedTypes[i];
                            if (_addNodesListSelectedIndex == i) GUI.color = Color.green;
                            ...
                            {
                                CreateAndAddNodeToCurrentDialog(_addNodesListSearchedTypes[_addNodesListSelectedIndex],
                                    _showAddNodesListPosition - nodesOffset);
                                HideAddNodesList();
        ...
        private void DrawMinimap()
        {
            ...
            using (new GroupBlock(zoomRect, GUIContent.none, "CN Box"))
            {
                ...
                foreach (var n in allNodesInRect)
                {
                    ...
                    EditorGUI.LabelField(scaledRect, GUIContent.none, "ColorPickerBox");
                }
            }
        ...
        private void DrawZoomIndicator()
        {
            ...
            EditorGUI.LabelField(zoomRect, GUIContent.none, "MiniMinMaxSliderHorizontal");
        ...
        private void PasteNodesFromClipboard()
        {
            ...
            foreach (var node in clipboard) clones.Add(EditorReflectionUtility.CreateDeepClone(node));
            ...
            else
            {
                foreach (var node in clones)
                    node.editorPosition =
                        new Vector2(node.editorPosition.x + 100f, node.editorPosition.y) - nodesOffset;
            }
            ...
            foreach (var kvp in lookupDict)
            {
                ...
                for (var edgeIndex = node.edges.Length - 1; edgeIndex >= 0; edgeIndex--)
                {
        ...
        {
            if (GetNodeBoxRect().Contains(Event.current.mousePosition))
                if (_isDrawingSelectionGrid == false && _isDraggingNode == false && _isDraggingEdge == false &&
                ...
                if (button == 0)
                {
                    if (_isDraggingEdge)
                    {
                        ...
                        foreach (var connectToNode in nodeEditors)
                        {
                            ...
                            if (connectToNode.node.index ==
                                _dragEdgeConnector.node.node.index) continue; // We can never connect to ourselves...
                    ...
                    if (_addNodesListRect.Contains(mousePos) == false && _showAddNodesList) HideAddNodesList();
                }
                else if (button == 1)
                {
                    var removed = RemoveEdgeAtPosition(mousePos);
                    if (removed)
                    {
                        OnEndDrag();
                        return;
...
//            }
        ...
        protected override void OnKeyUp(KeyCode keyCode)
        {
            ...
            if (keyCode == KeyCode.Delete ||
                ...
                if (selectedNodeEditors.Count > 0 &&
                    (EditorGUIUtility.editingTextField == false || GUIUtility.keyboardControl == -1))
                    if (EditorUtility.DisplayDialog("Deleting nodes",
                            "Are you sure you want to delete " + selectedNodeEditors.Count + " nodes?", "Yes",
                            "Cancel"))
                    {
        ...
        private void Zoom(float amount)
        {
            ...
            zoomAmount = (float)Math.Round((decimal)zoomAmount, 1);
        }
        ...
        private bool RemoveEdgeAtPosition(Vector2 position)
        {
            foreach (var node in nodeEditors)
            {
                ...
                for (uint i = 0; i < connectors.Length; i++)
                {
                    if (connectors[i].isHelperConnector) continue;
        ...
        {
            foreach (var node in nodes.Where(o => o.CanDelete())) RemoveNode(node.node);
        }
        ...
        private void DrawSidebar()
        {
            ...
            if (index != indexBefore) selectedSidebar = sidebars[index];

            GUI.Label(new Rect(10, 30f, sidebarWidth - 20f, EditorGUIUtility.singleLineHeight), _selectedSidebar.name,
        ...
        {
            foreach (var editor in selectedNodeEditors) editor.UnSelect();
        ...
        private void SelectNodes(List<NodeEditorBase> nodes)
        {
            ...
            {
                foreach (var node in selectedNodeEditors) node.UnSelect();

                selectedNodeEditors.Clear();
                selectedNodeEditors.AddRange(nodes);

                foreach (var node in selectedNodeEditors) node.Select();
        ...
        private NodeEditorBase GetNodeAtPosition(Vector2 pos)
        {
            ...
            for (var i = nodeEditors.Count - 1; i >= 0; i--)
            {
                var node = nodeEditors[i];
                if (ToZoomSpaceRect(node.GetNodeRect()).Contains(pos)) return node;
            }
        ...
        private Edge GetEdgeAtPosition(Vector2 pos)
        {
            ...
            {
                if (connector.isHelperConnector) continue;

                if (connector.index >= nodeEditor.node.edges.Length) continue;
...
//        }
        ...
        {
            Init();
            if (window != null) window.dialogueOwner = owner;
        }

        public static void Edit(Dialogue dialogue)
        {
            Init();
            if (window != null) window.dialogue = dialogue;
        }
    }
}