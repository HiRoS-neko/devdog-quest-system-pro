...
namespace Devdog.QuestSystemPro.Dialogue.Editors
{
    ...
    public class IfVariableEdgeConditionDrawer : ChildrenValueDrawerBase
    {
        ...
        protected override void DrawChildren(Rect rect)
        {
            if (hideGroup)
            {
                foreach (var child in children) child.Draw(ref rect);
            ...
            else
            {
                ...
                using (var foldout = new FoldoutBlock(this,
                           new Rect(rect.x, rect.y, rect.width, ReflectionDrawerStyles.singleLineHeight),
                           GUIContent.none))
                {
                    ...
                    if (foldout.isUnFolded)
                        ...
                        {
                            foreach (var child in children) child.Draw(ref indent.rect);
        ...
        protected void DrawVariableTypePicker(ref Rect rect)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                foreach (var sidebar in DialogueEditorWindow.sidebars)
                {
                    ...
                    if (s != null) s.Update(DialogueEditorWindow.window);
                }
            }
        }
    }
