    ...
    public static class QuestDecoratorUtility
    {
        ...
        public static string DrawQuestTaskField(Rect position, Quest quest, string taskName)
        {
            ...
            if (_questTaskPickerVal)
            {
                ...
                if (quest != null)
                {
                    var index = EditorGUI.Popup(position, IndexOf(quest.tasks, taskName),
                        quest.tasks.Select(o => o.key).ToArray());
                    if (index >= 0) taskName = quest.tasks[index].key;
                }
                else
                {
            ...
            if (EditorGUI.EndChangeCheck()) EditorPrefs.SetBool(QuestTaskPickerDropdownKey, _questTaskPickerVal);
